...
nv.models.stackedAreaBrushChart = function() {
  ...
  var margin = {top: 30, right: 25, bottom: 20, left: 60}
    , width = null
    , height = null
...
//TODO: consider deprecating and using multibar with single series for this
nv.models.verticalBar = function() {
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      wrap.attr('transform', 'translate(' + margin.left +height+ ',' + margin.top + ') , rotate(90)');
      //.attr("transform", "translate(" + margin.left +height + ", rotate(90)" )
      //------------------Add a title-----------------
      
      //var title = g.append('text').text("LIWC Category Scores");
      //title.attr('y',availableHeight);
...
// 
	 ...
	 var lblEnter = lbls.enter().append('text')
	          ...
	          .attr('fill', function(d,i) { return "gray"; })
	          .attr('fill-opacity', 1)
	          .attr('id',function(d,i) { return "nv-bar-text"+i; })
	          .attr('transform', function(d,i) {
	          	var yPosition=y(0)-5;
	          	if (getY(d,i)>=0) yPosition=y(0)+115;
	          	
	          	return 'translate(' + (x(getX(d,i))+ availableWidth / data[0].values.length * .45) + ','+yPosition+'), rotate(-90)'; }); //TODO: transition of labels
